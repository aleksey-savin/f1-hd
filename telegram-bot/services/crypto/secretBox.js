...
// to recover and replay to an external system (RouterOS credentials, port-knock
// sequence, IMAP/SMTP passwords, AI API keys).
// Storage format: "v1:<iv_b64>:<tag_b64>:<ciphertext_b64>".
//
// The key is dedicated — never JWT_SECRET — so leaking the auth-token secret
// does not expose stored passwords. It is read as APP_ENC_KEY (its real scope:
// the whole app) falling back to MIKROTIK_ENC_KEY, the historical name still
// present in every deployment's .env. The "v1" prefix leaves room for rotation.
...
const getKey = () => {
  ...
  if (!raw) {
    throw new Error("APP_ENC_KEY (or MIKROTIK_ENC_KEY) is not set");
  }

  const key = Buffer.from(raw, "base64");
  if (key.length !== 32) {
    throw new Error(
      `APP_ENC_KEY must decode to 32 bytes (got ${key.length})`,
    );
  }
