...
/**
 * Статус человека приходит из двух мест: из данных страницы (загрузчик, после
 * своего действия) и из живого табло присутствия (store/work-statuses —
 * автоматика по графику, ночной сброс, чужая правка). Берём более свежий:
 * табло не должно откатывать только что выбранный статус, а данные страницы —
 * прятать смену, случившуюся после загрузки.
 */
export const newerWorkStatus = (fromPage, live) =>
  live && new Date(live.updatedAt || 0) > new Date(fromPage?.updatedAt || 0)
    ? live
    : fromPage;
