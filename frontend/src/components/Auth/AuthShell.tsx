 ...
 *
 * Клетку на канве рисует не этот экран — она общая для приложения (index.css,
 * «Фактура канвы»). Класс `.auth-canvas` здесь только ПРИЗНАК экрана, как
 * `.mobile-shell` у app-shell: по нему канва добавляет мягкий свет за листом
 * формы. Свет живёт только тут, потому что якорь ему нужен видимый, а в
 * приложении за списком заявок такого якоря нет.
 */
...
const AuthShell = ({
  ...
  return (
    ...
    <div className="auth-canvas flex min-h-svh items-center">
      {/*
