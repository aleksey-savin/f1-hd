    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="xs"
          className="tw:ms-auto tw:font-medium tw:text-muted-foreground"
        >
          {sortBy.label}
...
const KnowledgeBaseExplorer = () => {
  ...
  return (
    ...
    <div className="tw:sticky tw:top-20 tw:flex tw:w-80 tw:flex-none tw:flex-col tw:gap-2 tw:xl:w-86">
      <div className="tw:flex tw:items-center tw:gap-2">
        ...
        {canManage && (
          <Button asChild size="icon" title="Новая заметка" aria-label="Новая заметка">
      ...
      {applied.length > 0 && (
        <div className="tw:flex tw:flex-wrap tw:items-center tw:gap-1.5 tw:rounded-xl tw:border tw:border-border tw:bg-card tw:px-2 tw:py-1.5">
          {applied.map((item) => (
            <button
              key={item.key}
              type="button"
      ...
      <div className="tw:flex tw:items-center tw:gap-2 tw:px-1 tw:text-sm tw:text-muted-foreground tw:tabular-nums">
        <span>Найдено: {filteredList.length}</span>
        {canReset && (
          <button
            type="button"
            onClick={resetFilter}
            className="tw:cursor-pointer tw:appearance-none tw:border-0 tw:bg-transparent tw:p-0 tw:text-sm tw:text-muted-foreground tw:underline tw:underline-offset-2 tw:hover:text-foreground"
          >
            Сбросить
          </button>
        )}
