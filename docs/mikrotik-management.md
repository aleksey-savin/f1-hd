...
- **UI**: в форме параметров — свитч **«Подключение через устройство»**, за
  которым скрыт селект **«Мост»** (`JumpBridgeField` в
  `MikrotikConnectionFields.jsx`; модалки размещают его после выбора компании).
  Кандидаты — настроенные записи **той же компании** (по `row.company.id` из
  DTO), без своего моста и кроме самой записи; компания не выбрана → список
  пуст; смена компании в standalone-форме сбрасывает выбранный мост; на
  страницах устройств модалка дозагружает список кандидатов (`store.fetch()`),
  если он пуст. Выключение свитча сбрасывает мост (= прямое подключение). При
  выбранном мосте knock-поля скрыты, а генератор инструкции не предлагает
  knock-пресеты. Строка «через <имя>» в таблице/мобильной
  карточке/`DeviceOverview`, поиск по имени моста.
...
- `markRecovered(record)` — closes the open episode (self-heals a missing one
  ...
  author = `Preferences.mikrotik.applicant` (the module's service account,
  email-pipeline comment pattern), pushed into
...
the tab shows when the inventory module is on) and authored by the module's
**service account `Preferences.mikrotik.applicant`** (picked at the top of the
tab, service accounts only; there is no system user, and the global
`defaultApplicant` is deliberately NOT used — unset ⇒ the module's tickets and
comments are skipped with a warn) with `source: "Мониторинг устройств"`, company
resolved from the device (falling back to the account's own company), deadline
from `Preferences.deadline`, and `notifications.pending` so the mailer delivers
them. Ticket text is human-readable (device name, host, last-seen,
...
`createMikrotikTicket` but with **company = the author's own company**: machine
tickets are authored by the module's service account
(`Preferences.mikrotik.applicant`), and its embedded `User.company` becomes the
ticket company — the account→company scheme used everywhere in the app. The devices span many companies, and a ticket with
**no** company at all breaks an app-wide invariant — the ticket page,
notifications and works dereference `ticket.company` unguarded, and mongoose
`toObject()` minimizes the empty object away → 500. Also **no
...
to the same author company when the device company can't be resolved.
...
- **Open** → sync: devices that left the endangered set (updated OR detached)
  ...
  `Preferences.mikrotik.applicant`. New endangered devices are `$push`ed into the checklist +
