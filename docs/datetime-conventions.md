...
- **Форматирование периода документа на фронте.** `periodFrom` отчёта — полночь
  ...
  готовой строкой (`row.period`, `services/reportCard`). Календарный месяц из
  строки `«2026-07»` форматируется `formatMonthLabel` — вообще без таймзоны:
  это не момент времени, и переводить его в чью-либо зону нельзя.
