...
module.exports = {
  ...
  // Флаг Secure у cookie сеанса — по СХЕМЕ публичного адреса, а не по NODE_ENV.
  // Стек публикует один голый HTTP-порт, TLS терминирует внешний прокси; пока
  // его нет (или адрес открыт по IP), браузер отбросил бы Secure-cookie, и вход
  // «проходил» бы, но не запоминался.
  useSecureCookies: /^https:/i.test(publicUrl),
  ...
  // Код из письма для входа. Пять минут по умолчанию плагина для этой
  // аудитории мало: почтовый крон ходит раз в десять секунд, но доставка до
  // внешнего ящика занимает минуты, а клиенты читают почту не мгновенно.
  // Срок читают и остров (плагин), и письмо (hooks.js) — источник один.
  loginCodeTtlSeconds: 15 * 60,
};
