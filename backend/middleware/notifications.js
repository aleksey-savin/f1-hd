...
exports.createTicketNotifications = async () => {
  ...
  for (let ticket of tickets) {
    ...
    switch (lastAction) {
      ...
      case "process ticket":
        // По закрытой заявке «вы назначены ответственным» не рассылаем:
        // правки задним числом (например, смена категории при сверке
        // финансовых отчётов) не должны уведомлять исполнителей.
        if (ticket.isClosed) {
          ticket.notifications.pending = false;
          await ticket.save();
          break;
