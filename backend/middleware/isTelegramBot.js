 ...
 *
 * Переходная поддержка `?api_token=` СНЯТА: отправщик шлёт секрет только
 * заголовком (`tg-service/src/api/client.ts`), а пока запасная дорога открыта,
 * любой токен, утёкший в старый `access.log`, остаётся рабочим ключом.
 */
