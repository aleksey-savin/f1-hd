...
// каталог править надо и потом: новое действие сотрудника обязано попасть в
// роль полного доступа, иначе `isFullAccess` перестанет её узнавать и зеркало
...
const run = async () => {
  ...
  // `staff` или `both`): по этому признаку зеркалится `isAdmin`, и роль без
  // одного такого действия погасила бы его. Клиентские действия в счёт не
  // идут — у сотрудника они не действуют.
  const full = roles.filter((role) =>
    access.isFullAccess(access.actionsToStatements(role.actions || [])),
  );
  if (!full.length) {
    throw new Error(
      "В каталоге нет ни одной роли, дающей весь словарь сотрудника. " +
  ...
  // Наборы прав ролей изменились — зеркало `isAdmin` у носителей обязано
  // догнать. Тем же кодом, что и правка роли из интерфейса.
  const { refreshMirrorFor } = require("@/services/roles");
  for (const row of written) {
    await refreshMirrorFor(String(orgId), row.key);
  }
