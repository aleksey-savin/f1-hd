...
const preferencesSchema = new Schema({
   ...
   *
   * ОТСРОЧКА ОБЯЗАТЕЛЬНА и не украшение: без неё администратор, щёлкнувший
   * переключатель, запирает снаружи в том числе себя — и починить это будет
   * некому, потому что снять требование может только он же. Пока идёт
   * отсрочка, вход работает, а портал показывает полосу с датой.
   *
   * «Администратор» здесь — роль, отдающая весь словарь прав (`isAdmin` её
   * зеркалит), а не должность.
   */
  twoFactorPolicy: {
    requireForAdmins: { type: Boolean, default: false },
    // До этого момента вход без второго фактора ещё разрешён. Проставляется
    // при включении требования, дальше не двигается сама.
  ...
  /**
   * Срез «давно без движения» на главной: с какого молчания заявка в него
   * попадает и какие заявки в зачёт не идут. Правила читает
   * services/ticketActivity, дефолты там продублированы — сервис зовут и до
   * того, как настройки хоть раз сохранили.
   *
   * Порог в РАБОЧИХ днях (производственный календарь): по календарным в
   * понедельник срез вспыхивает от субботы с воскресеньем.
   */
  staleTickets: {
    thresholdDays: { type: Number, default: 7, min: 1, max: 60 },
    // Мониторинг и заявки без заявителя: их разбирают пачкой
    ignoreAuto: { type: Boolean, default: true },
    // Они уже видны в своём срезе на главной
