...
// Серийный номер НЕ уникален — намеренно. Его печатает производитель, и в жизни
// он повторяется: партия одинаковых блоков питания с одним номером, нечитаемая
// наклейка, «--» вместо номера. Жёсткое ограничение люди обходили суффиксом
// («810354023088» и «810354023088\1» на дев-копии прода) — то есть портили
// данные, чтобы удовлетворить индекс. Совпадение теперь показывается формой как
// предупреждение со ссылкой на уже заведённое устройство, а сохранять или нет —
// решает человек. Индекс остаётся для поиска.
clientDeviceSchema.index({ serialNumber: 1 });
...
// попадают, поэтому много активов без инвентарного номера сосуществуют.
